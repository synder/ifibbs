...
const httpManager = require('./request/httpManager');
const GtConfig = require('./request/GtConfig');
const BatchImpl = require('./request/BatchImpl');
const RequestError = require('./request/RequestError');
...
function getFaster(_this) {
    let hosts = serviceMap[_this._appkey];
...
//    console.log("connect being invoked...");
    ...
    // 计算sign值
    let sign = utils.md5(this._appkey + timeStamp + this._masterSecret); //必须按顺序
    let postData = {
        action: 'connect',
        appkey: this._appkey,
        timeStamp: timeStamp,
...
GeTui.prototype.pushMessageToSingle = function (message, target, requestId, callback) {
    ...
    let postData = utils.createPostParams(message, target, requestId, this._appkey);
    this.httpPostJson(this._host, postData, false, callback);
};
...
GeTui.prototype.pushMessageToList = function (contentId, targetList, callback) {
    ...
    let needDetails = GtConfig.isPushListNeedDetails();
    let async = GtConfig.isPushListAsync();
    let limit;
    ...
    for (let idx in targetList) {
        ...
        let cid = target.getClientId();
        let alias = target.getAlias();
